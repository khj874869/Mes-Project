package com.mesproject.mescore.logging.logback;
...
public class KafkaLogbackAppender extends AppenderBase<ILoggingEvent> {
    ...
    private String service = "mes-core";
    ...
    public void setThreshold(String threshold) {
        if (threshold != null) this.threshold = Level.toLevel(threshold, Level.INFO);
    }

    @Override
    public void start() {
        if (bootstrapServers == null || bootstrapServers.isBlank()) {
            // Try env vars commonly used in docker / spring
    ...
    protected void append(ILoggingEvent event) {
        ...
        if (event.getLevel().isGreaterOrEqual(threshold) == false) return;

        // Prevent feedback loops
        String logger = event.getLoggerName();
        if (logger != null && (logger.startsWith("com.mesproject.mescore.logging") || logger.contains("KafkaLogbackAppender"))) {
            return;
        }
        ...
        payload.put("traceId", firstNonNull(
                MDC.get("traceId"),
        ...
        if (event.getThrowableProxy() != null) {
            payload.put("exception", event.getThrowableProxy().getClassName() + ": " + event.getThrowableProxy().getMessage());
        } else {
            payload.put("exception", null);
        }
